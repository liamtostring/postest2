...
function skipIfThese(requestUrl, currentUrl) {
    ...
    const pos_endpoint = new URL(location).searchParams.get('pos_endpoint');

    if ( currentUrl.includes("/"+pos_endpoint) ) {

        let _pathStr = requestUrl.split("/");
        if ( _pathStr.indexOf(pos_endpoint) > 0 || _pathStr.indexOf("uploads") > 0 || _pathStr.indexOf("static") > 0 ) {
            return true;
        } else {
            return false;
        }
