<?php
/**
 * POS most stocked report admin class file.
 *
...
/**
 * POS most stocked report admin class.
 */
class WC_Pos_Report_Most_Stocked extends WC_Report_Stock {
	...
	public function get_items( $current_page, $per_page, $outlet_id ) {
		...
		if ( $centralized_inventory_enabled ) {
			...
			OR (meta1.meta_value = 'yes' AND meta3.meta_key = '_stock' AND meta3.meta_value > $stock ) )";

		} else {
			$query .= ' AND posts.ID = mapped.product_id';
			$query .= " AND mapped.outlet_id = $outlet_id";
			$query .= " AND mapped.pos_stock > $stock";
		}
